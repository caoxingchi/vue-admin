  ...
  <div class="app-container">
    ...
    <div class="ccInfo">
      ...
      <div class="main">
        ...
        <p>
          <span
            >所属分类：{{ coursePublishInfo.firSubject }} —
            {{ coursePublishInfo.secSubject }}</span
          >
        </p>
        <p>课程讲师：{{ coursePublishInfo.teacherName }}</p>
        <h3 class="red">￥{{ coursePublishInfo.price }}元</h3>
      </div>
    </div>
    <div>
...
export default {
  ...
  created() {
    ...
    course.getCoursePublishInfo(this.courseId).then((resp) => {
      this.coursePublishInfo = resp.data.coursePublishInfo;
    });
  },
  methods: {
    previous() {
